// Send order confirmation email via Zoho SMTP.
// Invoked by:
//   - Admin "Resend Confirmation Email" button
//   - Storefront checkout after a successful order
//
// Uses SMTPS on smtp.zoho.com:465 (implicit TLS) with denomailer.

import { SMTPClient } from "https://deno.land/x/denomailer@1.6.0/mod.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
};

type Body = { order_id?: string; lang?: "ar" | "en" };

const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const SMTP_HOST = Deno.env.get("ZOHO_SMTP_HOST") ?? "smtp.zoho.com";
const SMTP_PORT = Number(Deno.env.get("ZOHO_SMTP_PORT") ?? "465");
const SMTP_USER = Deno.env.get("ZOHO_SMTP_USER")!;
const SMTP_PASS = Deno.env.get("ZOHO_SMTP_PASS")!;
const FROM_ADDRESS = Deno.env.get("ORDER_EMAIL_FROM_ADDRESS") ?? "no-reply@boutq.store";

function fmt(n: number, currency: string, lang: "ar" | "en") {
  try {
    return new Intl.NumberFormat(lang === "ar" ? "ar-BH" : "en-US", {
      style: "currency", currency, maximumFractionDigits: 3,
    }).format(Number(n || 0));
  } catch {
    return `${Number(n || 0).toFixed(3)} ${currency}`;
  }
}

function renderHtml(o: any, items: any[], settings: any, lang: "ar" | "en") {
  const isAr = lang === "ar";
  const dir = isAr ? "rtl" : "ltr";
  const brand = settings?.business_name ?? "Boutq";
  const primary = settings?.primary_color ?? "#111827";
  const L = isAr
    ? { greet: "شكراً لطلبك", intro: "تم استلام طلبك بنجاح. تفاصيله أدناه:", inv: "رقم الفاتورة", date: "التاريخ",
        item: "الصنف", qty: "الكمية", price: "السعر", total: "الإجمالي", subtotal: "المجموع الفرعي",
        shipping: "الشحن", grand: "الإجمالي النهائي", regards: "مع أطيب التحيات", footer: "هذه رسالة تلقائية، الرجاء عدم الرد." }
    : { greet: "Thanks for your order", intro: "We received your order. Details below:", inv: "Invoice #", date: "Date",
        item: "Item", qty: "Qty", price: "Price", total: "Total", subtotal: "Subtotal",
        shipping: "Shipping", grand: "Grand total", regards: "Warm regards", footer: "This is an automated message, please do not reply." };
  const cur = o.currency ?? "BHD";
  const rows = items.map((i: any) => `
    <tr>
      <td style="padding:10px 8px;border-bottom:1px solid #eee">${escapeHtml(i.description ?? "")}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:center">${i.quantity}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:${isAr ? "left" : "right"}">${fmt(Number(i.unit_price), cur, lang)}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:${isAr ? "left" : "right"}">${fmt(Number(i.line_total), cur, lang)}</td>
    </tr>`).join("");
  return `<!doctype html><html lang="${lang}" dir="${dir}"><head><meta charset="utf-8"><title>${escapeHtml(brand)} — ${L.inv} ${o.invoice_number}</title></head>
    <body style="margin:0;background:#f7f7f8;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#111">
      <div style="max-width:640px;margin:0 auto;background:#fff">
        <div style="padding:24px 28px;background:${primary};color:#fff">
          <h1 style="margin:0;font-size:22px">${escapeHtml(brand)}</h1>
          <p style="margin:6px 0 0;opacity:.9">${L.greet}</p>
        </div>
        <div style="padding:24px 28px">
          <p style="margin:0 0 16px">${L.intro}</p>
          <p style="margin:0 0 4px"><strong>${L.inv}:</strong> ${o.invoice_number}</p>
          <p style="margin:0 0 16px"><strong>${L.date}:</strong> ${new Date(o.order_date).toLocaleDateString(isAr ? "ar-BH" : "en-US")}</p>
          <table style="width:100%;border-collapse:collapse;margin-top:8px;font-size:14px">
            <thead><tr>
              <th style="padding:10px 8px;border-bottom:2px solid #111;text-align:${isAr ? "right" : "left"}">${L.item}</th>
              <th style="padding:10px 8px;border-bottom:2px solid #111;text-align:center">${L.qty}</th>
              <th style="padding:10px 8px;border-bottom:2px solid #111;text-align:${isAr ? "left" : "right"}">${L.price}</th>
              <th style="padding:10px 8px;border-bottom:2px solid #111;text-align:${isAr ? "left" : "right"}">${L.total}</th>
            </tr></thead>
            <tbody>${rows}</tbody>
          </table>
          <table style="width:100%;margin-top:16px;font-size:14px">
            <tr><td style="padding:4px 8px">${L.subtotal}</td><td style="padding:4px 8px;text-align:${isAr ? "left" : "right"}">${fmt(Number(o.subtotal), cur, lang)}</td></tr>
            ${Number(o.shipping) ? `<tr><td style="padding:4px 8px">${L.shipping}</td><td style="padding:4px 8px;text-align:${isAr ? "left" : "right"}">${fmt(Number(o.shipping), cur, lang)}</td></tr>` : ""}
            <tr><td style="padding:8px;font-weight:700;border-top:1px solid #ddd">${L.grand}</td><td style="padding:8px;font-weight:700;text-align:${isAr ? "left" : "right"};border-top:1px solid #ddd">${fmt(Number(o.total), cur, lang)}</td></tr>
          </table>
          <p style="margin:24px 0 0">${L.regards},<br/><strong>${escapeHtml(brand)}</strong></p>
        </div>
        <div style="padding:14px 28px;background:#fafafa;color:#888;font-size:12px;text-align:center">${L.footer}</div>
      </div>
    </body></html>`;
}

function escapeHtml(s: string) {
  return String(s ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]!));
}

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") return new Response(null, { headers: corsHeaders });
  if (req.method !== "POST") return new Response("Method not allowed", { status: 405, headers: corsHeaders });

  let body: Body = {};
  try { body = await req.json(); } catch { /* noop */ }
  const orderId = body.order_id;
  if (!orderId) return json({ error: "order_id required" }, 400);

  const admin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false, autoRefreshToken: false },
  });

  try {
    const { data: order, error: oe } = await admin
      .from("orders")
      .select("id, brand_id, invoice_number, order_date, subtotal, shipping, total, currency, customer_id")
      .eq("id", orderId)
      .maybeSingle();
    if (oe) throw oe;
    if (!order) return json({ error: "Order not found" }, 404);

    const [{ data: items }, { data: customer }, { data: settings }] = await Promise.all([
      admin.from("order_items").select("description, quantity, unit_price, line_total").eq("order_id", orderId),
      order.customer_id
        ? admin.from("customers").select("name, email").eq("id", order.customer_id).maybeSingle()
        : Promise.resolve({ data: null } as any),
      admin.from("business_settings").select("business_name, primary_color").eq("brand_id", order.brand_id).maybeSingle(),
    ]);

    const to = customer?.email?.trim();
    if (!to) {
      await admin.from("orders").update({
        confirmation_email_status: "failed",
        confirmation_email_error: "No customer email on file",
      }).eq("id", orderId);
      return json({ error: "No customer email" }, 400);
    }

    const lang: "ar" | "en" = body.lang === "ar" || body.lang === "en" ? body.lang : "en";
    const brandName = settings?.business_name ?? "Boutq";
    const subject = lang === "ar"
      ? `${brandName} — تأكيد الطلب #${order.invoice_number}`
      : `${brandName} — Order confirmation #${order.invoice_number}`;
    const html = renderHtml(order, items ?? [], settings, lang);

    const client = new SMTPClient({
      connection: {
        hostname: SMTP_HOST, port: SMTP_PORT, tls: true,
        auth: { username: SMTP_USER, password: SMTP_PASS },
      },
    });

    try {
      await client.send({
        from: `${brandName} <${FROM_ADDRESS}>`,
        to,
        subject,
        html,
        content: "auto",
      });
    } finally {
      await client.close();
    }

    await admin.from("orders").update({
      confirmation_email_status: "sent",
      confirmation_email_sent_at: new Date().toISOString(),
      confirmation_email_error: null,
    }).eq("id", orderId);

    return json({ ok: true });
  } catch (err: any) {
    console.error("[send-order-email] failed", err);
    const msg = String(err?.message ?? err ?? "Unknown error").slice(0, 500);
    try {
      await admin.from("orders").update({
        confirmation_email_status: "failed",
        confirmation_email_error: msg,
      }).eq("id", orderId);
    } catch { /* noop */ }
    return json({ error: msg }, 500);
  }
});

function json(data: unknown, status = 200) {
  return new Response(JSON.stringify(data), {
    status, headers: { "Content-Type": "application/json", ...corsHeaders },
  });
}
